...
Defines the hexagonal composition root for `kubectl-sql`. A single package `internal/app` owns the construction and wiring of every concrete adapter, injecting them as ports into the domain command constructors. The domain depends only on ports, and `cmd` is reduced to flag parsing and delegation. These requirements govern the structural contract that keeps adapters out of the domain and concentrates wiring in one place.
