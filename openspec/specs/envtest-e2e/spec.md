# Spec: envtest Integration Tests

## Purpose

Defines the behavioral contract for the envtest-backed integration test suite. These tests exercise the full SQL query path (parse → k8s LIST → table output) against a real in-process kube-apiserver without requiring a live cluster.
...
- **WHEN** fixture seeding completes
- **THEN** all seeded pods have `status.phase = Running` (set via status subresource PATCH)
