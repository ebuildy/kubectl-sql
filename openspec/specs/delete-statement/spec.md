# Spec: DELETE Statement

## Purpose

Defines the `DELETE` statement — the only mutating query in `kubectl-sql`. It covers the grammar
(including the MySQL-style hint comment for delete options and the trailing SELECT clauses), the
mandatory deletion-set preview, the interactive confirmation flow and `--yes` flag, bounded-parallel
deletion with a progress bar, the per-object result summary and exit-code contract, and DELETE's
interaction with `--watch`, `--dry-run`, and the REPL. The structural contract of the adapter that
executes DELETE is governed by the `sql-mutator-adapter` spec; the port's delete operation by the
`k8s-datasource-port` spec.
...
The query string SHALL accept a `DELETE [FROM] <resource> [<tail>]` statement, where `<resource>`
is any resolvable resource name (plural, short, or kind) and `<tail>` is an optional sequence of the
resolving SELECT's trailing clauses (`WHERE`, `ORDER BY`, `LIMIT`, `OFFSET`), each with the same
syntax and semantics as in a SELECT. The `<tail>` SHALL be forwarded verbatim to the resolving
`SELECT namespace, name FROM <resource> <tail>`, so a token after the resource that is not a
recognised SELECT clause keyword SHALL be a parse error. The `FROM` keyword SHALL be optional. The
statement SHALL be recognised case-insensitively and intercepted before the SELECT (octosql)
pipeline. A `DELETE` with no resource SHALL be a parse error and exit 1.
...
#### Scenario: DELETE with ORDER BY and LIMIT caps the deletion set
- **WHEN** the user runs `kubectl-sql "DELETE FROM pods ORDER BY name LIMIT 1"`
- **THEN** the trailing clauses are forwarded to the resolving SELECT, so exactly one object (the
  first by name) is matched and deleted
