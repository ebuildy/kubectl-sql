# Spec: SQL Mutator Adapter

## Purpose

Defines the structural contract of the `mutator` SQL adapter (`internal/adapter/sql/mutator`), the
sibling of the octosql adapter that owns mutating statements (DELETE now; UPDATE later). octosql is
SELECT-only, so the mutator is the single home for mutating-statement logic. It resolves the affected
objects by delegating a `SELECT namespace, name FROM <resource> [WHERE ...]` to the octosql engine,
performs mutations through the Kubernetes `DataSource` port, and applies deletions with bounded
parallelism. Observable DELETE behavior (preview, confirmation, exit codes) is governed by the
`delete-statement` spec.
