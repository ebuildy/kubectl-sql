# Spec: Query Typo Suggestion

## Purpose

Defines how a failed SQL query is turned into a high-confidence, single-token correction suggestion. When a query fails because of one mistyped SQL keyword, table name, or field — or an unterminated string literal — the engine proposes a corrected query rather than returning only the raw error, choosing the closest valid token by string similarity within a conservative threshold and confirming the correction before it runs.
