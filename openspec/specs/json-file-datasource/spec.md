# Spec: JSON File Datasource

## Purpose

Defines support for querying local JSON Lines files (`.json`, `.jsonl`, `.ndjson`) via
`FROM <path>`, alongside Kubernetes resources. The engine infers a column schema by
sampling the file, exposes the inferred top-level JSON keys as columns, and supports
`SELECT`, `WHERE`, `ORDER BY`, `LIMIT`, and all output formats identically to a
Kubernetes-backed table. Multiple JSON file tables can be combined via `JOIN`. Table
names follow path-based identifier syntax, including a `./` escape hatch for files
that would otherwise collide with the `k8s` database name. Files that are not valid
JSON Lines (one JSON object per line) cause the query to fail with a clear error.
