...
- A field present in both whose types are equal and `list` SHALL be merged recursively into its element `SubFields` (the list element schema), the same way object subfields are merged.
- A field present in both whose types disagree SHALL prefer the object form (enrichment): an object SHALL NOT be downgraded to a leaf, and a leaf SHALL be promoted to an object when the source is an object. A `list` SHALL NOT be downgraded to a scalar leaf; whichever side carries element `SubFields` SHALL be preserved.
...
#### Scenario: List element subfields are preserved through merging
- **WHEN** the embedded swagger snapshot supplies `spec->containers` as a list with element `SubFields` (Container fields), and a later layer also contributes `spec->containers`
- **THEN** the merged schema retains `spec->containers` as a list with its element `SubFields`, merged rather than dropped
...
#### Scenario: A scalar-element slice field is a String list
- **WHEN** the schema is inferred for a list field with no known element object schema (e.g. a `[]string`)
- **THEN** the field is `TypeIDList` with `Element` type `String`, not a struct or a map list (`Element` type `Any`)

#### Scenario: An object-element slice field is a Struct list
- **WHEN** the schema is inferred for `spec->containers` (whose element schema is known to be a Container)
- **THEN** the field is `TypeIDList` with `Element` type `TypeIDStruct` over the Container subfields (`name`, `image`, …)
